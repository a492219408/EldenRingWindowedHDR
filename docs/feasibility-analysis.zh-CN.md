...
分析日期：2026-08-31  
目标版本：App Ver. 1.17 / `eldenring.exe` 2.7.0.0  
阶段：0.6.0 已在首台 NVIDIA 实机通过无边框/普通窗口化 HDR、SDR 恢复与开关持久化；
0.6.1 完成最终审计补丁；1.0.0 正在整理正式发行物
...
运行时会在安装 Hook 前校验目标 EXE 的大小和 SHA-256。ModEngine3 配置使用提前加载与
显式 initializer；每次启动的日志写在 DLL 同目录，具体测试矩阵见
...
本轮收集时未传 `-GameExePath`，只使 `system.txt` 少了脚本冗余采集的 EXE 元数据。DLL 自身
在 Hook 前记录并强校验了 `87,024,720` 字节和固定 SHA-256；这与只读备份复算结果一致，
因此不影响测试有效性。
...
所有地址和字节结论只适用于该哈希。游戏更新后必须重新扫描和验证控制流。
...
`unlock_hdr_menu` 仍先调用原函数用于诊断，再仅返回 `false`。安装前校验完整 EXE 指纹、
RTTI、五个相邻虚表项和 19 字节原函数体。若行为修改模式发现未知前置 Hook 或字节变更，
则拒绝覆盖并保留 DXGI/AGS 观测。
...
0.6.0 在完整 EXE 哈希通过后校验该函数 14 字节入口。入口含 RIP-relative 安全 cookie
读取，不能直接复制到任意地址，因此实现使用专用 trampoline：把 cookie 地址改写为绝对
...
- Alt+Tab、Windows HDR 热切换、休眠恢复和设备重建后的稳定性；
- 两台显示器 HDR 状态不一致时的跨屏/移动，以及显示器断连；
- AMD、Intel 以及其他 NVIDIA 驱动/显示器组合；
- Overlay、ReShade、OBS、UnlockTheFps 和其他 DXGI Hook 的共存；
...
| Alt+Tab、Resize、显示器切换后失效 | 中高 | Resize 有基本状态机覆盖；双 HDR 显示器移动已通过视觉测试，但输出能力变化不一定触发 revision，其他场景仍需实测。 |
| AMD AGS 与现代 DXGI 状态冲突 | 中高 | 只复现游戏自己的 AMD 时序；其他厂商不调用 AGS。 |
| Windows HDR 未开启 | 中 | 不擅自改全局设置，给出明确日志并回退。 |
| 游戏更新与 Arxan 影响签名 | 高 | 哈希门控、可执行节唯一扫描、上下文验证。 |
...
- **部分满足：**正常退出、已测 Resize、Windows HDR 关闭回退和双 HDR 显示器移动成立；
  Alt+Tab、HDR/SDR 混合显示器及系统 HDR 热切换仍需扩展验证。
...
- Rust + ModEngine3 DLL 方案：高。0.1.0 至 0.6.0 已真实加载并逐层验证；当前 NVIDIA
  环境已得到正常无边框/普通窗口化 HDR、正常 SDR 恢复和跨启动持久化。
