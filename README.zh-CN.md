...
这是一个由 ModEngine3 提前加载的 Rust DLL，用于实现《艾尔登法环》的窗口态 HDR。早期
...
最终跨版本回归又确认 App Ver. 1.16.2 与 1.17 都能由同一运行时解析器定位目标，并在真实
游戏中完成窗口态 HDR/SDR、启动持久化与普通窗口化测试。

这仍不是不受条件限制的通用 HDR MOD。现有运行时证据来自一台 NVIDIA 测试机，覆盖上述
两个游戏版本；维护者已
...
启动自动恢复。1.0.0 的四次跨版本实测确认动态解析器在 1.16.2 与 1.17 中均得到唯一且与
已知地址一致的目标，受管 PQ/SDR 转换全部成功；Alt+Tab 与同时加载 UnlockTheFps 也未出现
问题。维护者后续又确认 Windows HDR 关闭时选项安全保持不可用，两台 HDR 显示器
